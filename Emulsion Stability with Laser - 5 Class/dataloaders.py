...
class SolubilityDataset(Dataset):
    def __init__(self, root, image_paths, targets, transform=None):
        ...
        self.class_to_idx = {"separated": 0, "stable": 1, "separationstarted": 2, "possibleseparated": 3, "notapplicable": 4}
        self.idx_to_class = [ "separated", "stable", "separationstarted", "possibleseparated", "notapplicable"]
        # Normalize targets: strip spaces and lowercase
        self.targets = [self.class_to_idx[target.strip().lower()] for target in targets]
...
def prep_data(train_df, val_df, test_df, config):
    ...
    class_counts = np.bincount(train_dataset.targets, minlength=5)
