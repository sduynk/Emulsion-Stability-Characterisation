...
def compute_confusion_matrix(dataloader, model, device, num_classes=3): #seda
    model.eval()
    all_labels = []
    all_preds = []
...
def test_model(dataloader, model, device):
    ...
    metric_dictionary = metrics(all_labels, all_preds, 3)
...
def train_model(train_dataloader, val_dataloader, model, optimizer, scheduler, device, num_epochs=30):
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics["F1@3"] > best_f1:
            best_f1 = val_metrics["F1@3"]
            best_model_wts = copy.deepcopy(model.state_dict())
...
def train_cross_val(config, save_path=None):
    ...
    for fold, (train_loader, val_loader, test_loader) in enumerate(cross_val_solubility(config)):
        ...
        test_cm = compute_confusion_matrix(test_loader, model, device, num_classes=3)
        print(f"\n[Fold {fold}] TEST confusion matrix (rows=true, cols=pred):\n{test_cm}\n")

        # Misclassified samples
        misclassified_df = collect_misclassified(test_loader, model, device, num_classes=3)
...
def collect_misclassified(dataloader, model, device, num_classes=3):
    model.eval()
    rows = []
