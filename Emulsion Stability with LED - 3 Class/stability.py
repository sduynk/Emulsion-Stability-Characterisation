...
def run(config):
    out = []
    cms_sum = np.zeros((3,3), dtype=np.int64)  # total CM across all folds+seeds
    cms_count = 0
    ...
    for i in range(0, 3):
        ...
        for fold in range(6):  # n_splits=6 olduğu için
    ...
    if cms_count > 0:
        cm_df = pd.DataFrame(
            cms_sum,
            index=["true_separated","true_stable","true_separationstarted"],
            columns=["pred_separated","pred_stable","pred_separationstarted"]
        )
...
def main():
    ...
    config = {
        "data_dir": r"C:\Users\sedau\Desktop\emulsion_stability - led 3 class\groupid216_split_crop_merge_labelcorrection",
        "model": None,
        "lr": 0.0005,
        "batch_size": 128,
        "weight_decay": 0.001,
        "center_crop": (1080, 1230),
        "resize": (224, 224),
        "degrees": 20,
        "translate": (0.1, 0.1),
        "scale_lower": 0.95,
